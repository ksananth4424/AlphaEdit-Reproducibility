...
from glue_eval.boolq_eval import BoolQEval
from glue_eval.hellaswag_eval import HellaSwagEval
from glue_eval.math_eval import MATHEval
from glue_eval.xstest_eval import XSTestEval


class GLUEEval():
    def __init__(self, model, tokenizer, number_of_tests = None, sst_number_of_few_shots = 0, mrpc_number_of_few_shots = 0, cola_number_of_few_shots = 0, rte_number_of_few_shots = 0, mmlu_number_of_few_shots = 0, sentiment_analysis_number_of_few_shots = 0, nli_number_of_few_shots = 0, dialogue_number_of_few_shots = 0, boolq_number_of_few_shots = 0, hellaswag_number_of_few_shots = 0, math_number_of_few_shots = 0, xstest_number_of_few_shots = 0):
        self.model = model
        ...
        self.boolq_eval = BoolQEval(model, tokenizer, number_of_tests = number_of_tests, number_of_few_shots = boolq_number_of_few_shots)

        self.hellaswag_eval = HellaSwagEval(model, tokenizer, number_of_tests = number_of_tests, number_of_few_shots = hellaswag_number_of_few_shots)

        self.math_eval = MATHEval(model, tokenizer, number_of_tests = number_of_tests, number_of_few_shots = math_number_of_few_shots)

        self.xstest_eval = XSTestEval(model, tokenizer, number_of_tests = number_of_tests, number_of_few_shots = xstest_number_of_few_shots)
    ...
    def evaluate(self, glue_results, record_path, perplexity_flag = False, sst_flag = False, mmlu_flag = False, mrpc_flag = False, cola_flag = False, rte_flag = False, nli_flag = False, sentiment_analysis_flag = False, dialogue_flag = False, boolq_flag = False, hellaswag_flag = False, math_flag = False, xstest_flag = False, gen_len = 5):
        if perplexity_flag:
            raw_ds = load_dataset(
                        "wikitext",
        ...
        if boolq_flag:
            ...
            glue_results['boolq'] = result_dict
        ...
        if hellaswag_flag:
            ...
            self._save_generations(record_path, generations, 'hellaswag')

        if math_flag:
            result_dict, generations = self.math_eval.evaluate(gen_len)
            glue_results['math'] = result_dict
            self._save_generations(record_path, generations, 'math')

        if xstest_flag:
            result_dict, generations = self.xstest_eval.evaluate(gen_len)
            glue_results['xstest'] = result_dict
            self._save_generations(record_path, generations, 'xstest')
