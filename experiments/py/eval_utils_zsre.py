...
def _tokenize_target(tok, target_str):
    ids = tok(target_str)["input_ids"]
    if (
...
def _prefix_len(tok, prompt_str):
...
def compute_rewrite_quality_zsre(
    ...
    target_tok = _tokenize_target(tok, " " + target_new["str"])
...
def test_batch_prediction_acc(model, tok, prompts: typing.List[str], target):
    ...
    with torch.no_grad():
        ...
        correct_id = tok(target, padding=True, return_tensors="pt").to(_model_device(model))[
            "input_ids"
        ]
        ...
        correct_ids = tok(
            ...
            padding=True,
            return_tensors="pt"
        ).to(_model_device(model))["input_ids"]

        if (
            tok.bos_token_id is not None
            and correct_ids.size(1) > 1
            and (correct_ids[:, 0] == tok.bos_token_id).all()
        ):
            correct_id = correct_ids[:, 1]
        else:
            correct_id = correct_ids[:, 0]

        correct_id = correct_id.squeeze()
