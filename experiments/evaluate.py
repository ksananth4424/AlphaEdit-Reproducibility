...
from util.device import is_sharded_device_spec
...
def main(
    ...
    if type(model_name) is str:
        ...
        if "Qwen" in model_name or "qwen" in model_name:
            ...
            if is_sharded_device_spec(device):
                model = AutoModelForCausalLM.from_pretrained(
                    model_name,
                    device_map="auto"
                )
        ...
        # # Determine whether the FFN output weight is (d_model, d_mlp) or (d_mlp, d_model)
        #         # and size the covariance accordingly. GPT-2-XL uses the transposed layout.
    ...
    if any(alg in alg_name for alg in ["AlphaEdit", "MEMIT_seq", "MEMIT_prune", "NSE"]):
        ...
        elif hparams.model_name in ["EleutherAI_gpt-j-6B","Llama3-8B","phi-1.5","Llama3.2-1B","Llama3.2-3B","Qwen2.5-3B","Gemma2-2B","Phi3-mini-3.8B"]:
    ...
    for record_chunks in chunks(ds, num_edits):
        ...
        if cnt == 0 and args.downstream_eval_steps > 0:#do initial GLUE EVAL WITH ORIGINAL MODEL
            ...
            # glue_results = glue_eval.evaluate(glue_results, out_file, nli_flag = True, sst_flag = True, cola_flag=True, rte_flag=True, mmlu_flag = True, mrpc_flag = True, boolq_flag = True, hellaswag_flag = True, math_flag = True, xstest_flag = True)

            model.config.use_cache = True          # re-enable KV cache for generation
            with torch.inference_mode():           # no grad graph overhead
                glue_eval = GLUEEval(model, tok, number_of_tests=100)
                glue_results = glue_eval.evaluate(
                    ...
                    boolq_flag=True, hellaswag_flag=True,
                    math_flag=True, xstest_flag=True
                )
            model.config.use_cache = False 
